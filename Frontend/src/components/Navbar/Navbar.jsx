...
const Navbar = () => {
    ...
    return (
        <>
            <nav className="z-50 relative flex bg-gray-900 text-gray-100 px-6 py-3 items-center justify-between shadow-lg">
                <div className="flex items-center">
                    <img src={logo} alt="Warfare Bookshelf Logo" className="h-8 w-8 mr-3" />
            ...
            <div className={`${MobileNav} bg-zinc-800 h-screen absolute top-0 left-0 w-full z-40 flex flex-col items-center justify-center`}>
                {links.map((items, i) => (
                    <Link to={items.link}
                        className={`${MobileNav} text-white text-4xl font-semibold hover:text-blue-500 transition-all duration-300 mb-8`}
                        key={i}
                ...
                {isLoggedIn === false && (
                    <>
                        <Link to="/LogIn" className={`${MobileNav} px-8 mb-8 text-3xl font-semibold py-2 border text-white border-blue-500 rounded hover:text-zinc-800 hover:bg-white transition-all duration-300`}>LogIn</Link>
                        <Link to="/SignUp" className={`${MobileNav} px-8 py-2 mb-8 bg-blue-500 text-3xl font-semibold rounded hover:bg-white hover:text-zinc-800 transition-all duration-300`}>SignUp</Link>
